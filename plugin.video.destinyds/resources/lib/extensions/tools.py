...
class System(object):
	...
	GaiaAddon = 'plugin.video.destinyds'
	...
	def restart(self, sleep = True):
		...
		System.execute('RunAddon(%s)' % 'plugin.video.destinyds')
	...
	@classmethod
	def info(self, value):
		return xbmcaddon.Addon('plugin.video.destinyds').getAddonInfo(value)
	...
	def window(self, action = None, parameters = {}, command = None, sleep = None, refresh = False):
		if command == None:
			...
			if not parameters == None and not parameters == '' and not parameters == {}:
				...
				parameters = ''
			command = '%s%s/%s' % (System.PluginPrefix, 'plugin.video.destinyds', parameters)
...
SettingsAddon = xbmcaddon.Addon('plugin.video.destinyds')
...
class Media(object):
	...
	NameSeasonLong = 'season'#xbmcaddon.Addon('plugin.video.destinyds').getLocalizedString(32055).encode('utf-8')
	NameSeasonShort = NameSeasonLong[0].upper()
	NameEpisodeLong = 'S'#xbmcaddon.Addon('plugin.video.destinyds').getLocalizedString(33028).encode('utf-8')
